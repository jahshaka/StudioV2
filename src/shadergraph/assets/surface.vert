...
void main()
{
	...
	vec4 localPos = vec4(a_pos + vertexOffset + (vertexExtrusion * v_localNormal),1.0);
    v_worldPos = (u_worldMatrix * localPos).xyz;
    gl_Position = u_projMatrix * u_viewMatrix * u_worldMatrix * localPos;
}
